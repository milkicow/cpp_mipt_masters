...
template <typename D, typename T, bool C = is_empty_v<D>>
class Has {};
...
class Has<D, T, false> {
   private:
    ...
    static constexpr auto current_match = std::is_same_v<current_t, T>;
    static constexpr auto rest_match = Has<rest_t, T>::value;

   public:
    static constexpr auto value = current_match || rest_match;
};

template <typename D, typename T>
constexpr auto has_v = Has<D, T>::value;
...
int main() {
    ...
    static_assert(has_v<Deque<int, double>, int>);

    static_assert(has_v<Deque<int, double>, double>);

    static_assert(!has_v<Deque<int, double>, float>);
}
